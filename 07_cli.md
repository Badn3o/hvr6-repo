...
In this method, the HVR commands are executed directly/locally on the hub machine. The authentication to the HVR Hub System is done using the credentials of the user logged in to a hub machine. In this case, only the hub name is needed to run a command (e.g.`hvractivate``myhub mychn`).
